...
from models import Book, Author, book_schema


def read_one(book_id):
    note = Book.query.get(book_id)

    if note is not None:
        return book_schema.dump(note)
    else:
        abort(
            404, f"Note with ID {book_id} not found"
        )
...
def create(book):
    ...
    if author:
        ...
        abort(
            404,
            f"Person not found for ID: {author_id}"
        )
